...
- **Danh sách dữ liệu media**
  ...
  - Đơn giá / Tỷ lệ chia — *Đọc* — từ thông tin khi tạo **Media ID** này *(bản gốc ghi nhầm "ID quảng cáo")*
    - Sửa
      - Có hiệu lực hiện tại
      - Có hiệu lực về sau
  ...
  - Số tiền phải trả — *Tính toán*
    - Đơn giá × Dữ liệu **lưu lượng** × Hệ số dữ liệu → khi có dữ liệu quyết toán / số tiền: Đơn giá × Dữ liệu quyết toán × Hệ số dữ liệu
    - Tỷ lệ chia × Số tiền × Hệ số dữ liệu → khi có dữ liệu quyết toán / số tiền: Tỷ lệ chia × Số tiền quyết toán × Hệ số dữ liệu
    - **Đính chính:** bản gốc ghi "Đơn giá × dữ liệu quyết toán × hệ số" ngay cả khi *chưa* có quyết toán (không hợp lý — lúc đó chỉ có dữ liệu lưu lượng), và công thức sau quyết toán bị mất hệ số dữ liệu. Đã sửa: trước quyết toán dùng lưu lượng, hệ số dữ liệu áp dụng ở cả hai giai đoạn.
  - Tỷ lệ chia tài khoản — *Đọc* — từ thông tin khi tạo **Media ID** này *(bản gốc ghi nhầm "ID quảng cáo")*
    - Sửa
      - Có hiệu lực hiện tại
      - Có hiệu lực về sau
...
- **Danh sách dữ liệu media**
  ...
  - Số tiền thực trả — *Đọc* — từ quản lý dữ liệu media *(bản gốc ghi "thực thu" 实收 — nhầm; đây là tiền trả cho media, đúng phải là 实付 "thực trả")*
  - Trạng thái — *Đọc* — từ trạng thái ID hiện tại
...
## PHỤ LỤC — TỔNG HỢP ĐÍNH CHÍNH SO VỚI BẢN GỐC TIẾNG TRUNG

| # | Vị trí | Bản gốc (CN) | Sửa thành | Lý do |
|---|--------|--------------|-----------|-------|
| 1 | Tạo ID quảng cáo / Media / Media ID | "广告主唯一" (nhà QC duy nhất) | Thực thể tương ứng là duy nhất | Lỗi copy-paste từ mục nhà QC |
| 2 | Tạo Media ID / Đơn QC media | Tất cả field đều "从广告主列表选择" | Chọn từ danh sách tương ứng (Đơn QC, ID QC, Media, Đơn QC media…) | Lỗi copy-paste |
| 3 | Quản lý dữ liệu media — Đơn giá/Tỷ lệ chia, Tỷ lệ chia TK | "来自创建此广告ID时的信息" | Từ thông tin khi tạo **Media ID** | Cột thuộc Media ID, không phải Ad ID |
| 4 | Số tiền phải trả (trước quyết toán) | Đơn giá × dữ liệu quyết toán × hệ số | Đơn giá × dữ liệu **lưu lượng** × hệ số | Chưa quyết toán thì chỉ có dữ liệu lưu lượng |
| 5 | Số tiền phải trả (sau quyết toán) | Bỏ mất hệ số dữ liệu | Giữ × hệ số dữ liệu | Hệ số áp dụng cho mọi giai đoạn |
| 6 | Số tiền thực trả | 应收金额 × 分账比例 (phải **thu**) | Số tiền phải **trả** × tỷ lệ chia TK | Nhánh media là tiền chi trả |
| 7 | Tra cứu dữ liệu media | 实收金额 (thực **thu**) | Số tiền thực **trả** | Nhất quán với Quản lý dữ liệu media |
| 8 | Công thức CPM | Đơn giá × lưu lượng | Đơn giá × lưu lượng **/ 1000** | Chuẩn ngành CPM (per mille) |
